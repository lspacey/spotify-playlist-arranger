...
import os, sys, time, threading, collections, json, pathlib, datetime
import numpy as np

os.environ["HF_HUB_DISABLE_SYMLINKS_WARNING"] = "1"
os.environ["HF_HUB_CACHE"] = r"C:\Users\lspacey\.cache\huggingface\hub"
...
def show_playlists_table(console, playlists):
    """Display playlist table, return chosen 0-based index."""
    ...
    console.print(table)
    while True:
        choice = IntPrompt.ask("[bold yellow]Select playlist #[/bold yellow]", default=1)
...
def show_playlist_menu(console, tracks, db):
    ...
    try:
        while True:
            ...
            while True:
                ...
                action = show_playlist_menu(console, tracks, db)
                ...
                if action == "back":
                    ...
                    console.print(
                        "\n[bold yellow]Choose anchors and reorder:[/bold yellow] "
                        "[dim]not yet implemented — returning to playlist list.[/dim]"
                    )
                    break
